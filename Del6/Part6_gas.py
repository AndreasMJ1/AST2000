...
def GaussianModel(lambda0,mass):
    """
    Function for analyzing and plotting spectral lines in target planets atmosphere
    """
    ...
    tol = 1e-3
    ### Finding index at max and min point in array ##
    ...
    lowest = 700000 #Arbitrary high value 

    ### looping through relevant values ###
    for i in range(len(lambda_range)):
        for j in range(len(Temps)):
            sigma = sigmasolver(lambdas[ind0:ind1]*1e9,mass,Temps[j]) 
